...
impl RRTChecksum for ChecksumV00 {
    ...
    // TODO: I would prefer to have that as
    // fn size_of(&self) -> usize {
    //     mem::size_of::<ChecksumV00>()
    // }
}
...
mod tests {
    ...
    // #[test]
    // fn it_gets_size() {
    //     assert_eq!(20, mem::size_of::<ChecksumV00>());
    // }

    #[test]
    fn it_calculates() {
        let mut checksum = ChecksumV00::new();
        assert_eq!(checksum.calculate("A".as_bytes()), [0, 78]);
    }

    #[test]
    fn it_calculate() {
        let mut checksum = ChecksumV00::new();
        assert_eq!(checksum.calculate(b"A"), [0, 78]);
    }
    ...
    fn it_calculates_from_str() {
        ...
        assert_eq!(checksum.calculate(b"0"), [0, 87]);
        assert_eq!(checksum.calculate(b"1"), [0, 88]);
        assert_eq!(checksum.calculate(b"A"), [0, 78]);
        assert_eq!(checksum.calculate(b"AA"), [0, 65]);
        assert_eq!(checksum.calculate(b"AB09"), [0, 67]);
        assert_eq!(checksum.calculate(b"ZZZZZZ"), [0, 67]);
        assert_eq!(checksum.calculate(b"010012345TWBABAEFGH"), [0, 74]);
    }
    ...
    fn it_has_a_checksum_always_between_65_and_90() {
        ...
        for _ in 1..26 * 100 {
            s += "A";
            // println!("{}", s);
            let c = checksum.calculate(s.as_bytes())[1];
            assert!(c >= 65); // Ascii A
            assert!(c <= 90); // Ascii Z
        }
    }
